# Resize — AI agent context

A small SwiftUI iOS app that converts and resizes photos and writes the outputs to a user album called "Resize". Single-user, fully offline, no analytics, no accounts, no payments.
...
- File System Synchronized project (Xcode 16+): drop new `.swift` files into `Resize/`, `ResizeTests/`, or `ResizeUITests/` and the build picks them up — no `project.pbxproj` edits needed.

## Layout

```
Resize/
├─ ResizeApp.swift           # @main, owns @State ResizeSession + ResizeRouter
├─ Core/                     # Models + pipeline (no UI)
│  ├─ OutputFormat.swift     # enum: jpeg|webp|avif|png|tiff + UTType/MIME/quality flag
│  ├─ ResizeMode.swift       # enum: original|percentage|longEdge|shortEdge + targetMaxPixelSize(for:source:)
│  ├─ ResizeSession.swift    # @Observable: assets, format, quality, resize, progress, failures
│  └─ ImageProcessor.swift   # nonisolated static decode/resize/encode pipeline
├─ Photos/
...
└─ UI/
   ...
   ├─ ResizeOptionsView.swift# 4/4 Resize mode + inline pixel field
   ├─ ProcessingView.swift   # Bounded TaskGroup + ProgressRing + Cancel
   ├─ DoneView.swift         # Summary + Open Photos + Resize More + Retry
   └─ Components/
      ├─ StepDots.swift
      ├─ OptionRow.swift
      ├─ PrimaryCTA.swift
...
- `ResizeMode.targetMaxPixelSize` math across every mode + aspect ratio
...
1. Fresh install. Launch → Home screen shows "Resize" / "Convert and resize photos in a tap." / hero card / "Choose Photos" CTA.
...
7. **Resize** (4/4): pick **Long edge**, type `2000`. Tap **Process 3 Photos**.
8. **Processing**: ring animates 0 → 100 %. Don't tap Cancel.
9. **Done**: "3 photos saved to Resize." Tap **Open Photos**.
10. In Photos → Albums → "Resize" verify:
    - 3 JPEGs present
...
11. Return to Resize → **Resize More** pops to Home with session reset.
...
14. Mid-processing on a larger batch, tap **Cancel**: returns to Resize step; already-saved outputs stay in the album (no rollback).

## App icon

`scripts/generate_icon.swift` regenerates the three 1024×1024 variants (default/dark/tinted) into `Resize/Assets.xcassets/AppIcon.appiconset/`. Edit `IconView` inside that script and rerun `swift scripts/generate_icon.swift` to update all three at once.
